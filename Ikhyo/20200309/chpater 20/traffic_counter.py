...
ap.add_argument("-m","--mode",type = str,required=True,choices=["horizontal","vertical"],help ="direction in which vehicles will be moving")
...
while True:
    ...
    if start:
        if W is None or H is None:
            fps = FPS().start()
            (H,W) = frame.shape[:2]
            dc = DirectionCounter(args["mode"],W - conf["x_offset"],H - conf["y_offset"])
        ...
        if args["mode"] == "vertical":
            cv2.line(frame,(0,H - conf["y_offset"]),(W,H - conf["y_offset"]),(0,255,255),2)
            if diffPt is not None:
                cv2.line(frame,(diffPt,0),(diffPt,H),(255,0,0),2)
        else:
            cv2.line(frame,(W-conf["x_offset"],0),(W-conf["x_offset"],H),(0,255,255),2)
            if diffPt is not None:
